...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        """
        Tìm PII trong value; nếu phát hiện, redact và trả về kết quả đã xử lý.
        ...
        Bước:
          ...
          3. Nếu found_pii không rỗng → FailResult(fix_value=redacted_text)
             (với OnFailAction.FIX, Guardrails sẽ thay output bằng fix_value)
          4. Nếu không tìm thấy PII → PassResult (text gốc đã an toàn)
        """
        redacted_text = value
        found_pii     = []
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            # JSON đã hợp lệ — chỉ format lại cho đẹp nếu cần.
            if formatted == value:
                return PassResult()
            return FailResult(
                error_message="JSON hợp lệ nhưng chưa được format chuẩn",
                fix_value=formatted,
            )
        ...
        try:
            ...
            # Với OnFailAction.FIX, fix_value sẽ thay thế output lỗi.
            return FailResult(
